...
<body class="text-slate-200 min-h-screen antialiased">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <!-- VISTA 3: BALANCE DE PAGOS (CON ALTURA ESTABLE FIJA) -->
        <div id="view-pagos" class="tab-view hidden space-y-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
                
                <!-- Tarjeta: Efectivo vs Yape -->
                <div class="premium-card rounded-2xl p-6 shadow-xl flex flex-col h-full">
                    ...
                    <!-- CORREGIDO: h-[280px] asegura que la caja no colapse y sea perfectamente deslizable -->
                    <div class="w-full">
                        <h4 class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">Desglose por Tutor</h4>
                        <div class="space-y-2 h-[280px] overflow-y-auto pr-2 custom-scroll" id="list-efectivo-yape"></div>
                    </div>
                </div>

                <!-- Tarjeta: Pagantes vs Meta -->
                <div class="premium-card rounded-2xl p-6 shadow-xl flex flex-col h-full">
                    <div class="border-b border-slate-800/80 pb-3 mb-5">
                    ...
                    <div class="space-y-3 mb-6">
                        <div class="flex justify-between items-center bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/40">
                            ...
                            <span class="text-lg font-bold text-orange-400" id="box-meta-alumnos">...</span>
                        </div>
                        <div class="flex justify-between items-center bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/40">
                    ...
                    <!-- CORREGIDO: h-[280px] para control simétrico deslizable estable -->
                    <div class="w-full">
                        <h4 class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">Desglose por Tutor</h4>
                        <div class="space-y-2 h-[280px] overflow-y-auto pr-2 custom-scroll" id="list-pagantes-meta"></div>
                    </div>
                </div>
    ...
    <script>
        ...
        function borderAlphaFix(index) {
            return index === 0 ? 'bg-amber-500/10 border-amber-500/30' : 
                   ...
                   index === 2 ? 'bg-amber-700/10 border-amber-700/30' : 'bg-slate-900/60 border-slate-800/80';
        }
        ...
        function renderPagosLists(data) {
            ...
            data.forEach(row => {
                ...
                itemEf.className = "bg-slate-950/60 p-3 rounded-xl border border-slate-800 flex justify-between items-center hover:bg-slate-900/40 transition-colors mb-2";
                itemEf.innerHTML = `
                    <div class="truncate pr-2">
                        <p class="text-[11px] font-bold text-slate-200 truncate">${row.tutor}</p>
                        <p class="text-[9px] text-slate-500">${row.ciclo}</p>
                    </div>
                    <div class="text-right flex-shrink-0 flex gap-4">
                        <div class="text-right">
                            <p class="text-[9px] text-slate-500 uppercase font-semibold">Efectivo</p>
                            <p class="text-xs font-bold text-teal-400">S/ ${row.efectivo.toLocaleString('es-PE', {minimumFractionDigits:0})}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-[9px] text-slate-500 uppercase font-semibold">Yape</p>
                            <p class="text-xs font-bold text-indigo-400">S/ ${row.yape.toLocaleString('es-PE', {minimumFractionDigits:0})}</p>
                        </div>
                    </div>
                `;
                ...
                itemPag.className = "bg-slate-950/60 p-3 rounded-xl border border-slate-800 flex justify-between items-center hover:bg-slate-900/40 transition-colors mb-2";
                
                let faltaText = row.falta < 0 ? 'Sobran' : 'Faltan';
                let faltaColor = row.falta < 0 ? 'text-emerald-400' : 'text-rose-400';
                let faltaVal = row.metaEst - row.pagantes;
                ...
                itemPag.innerHTML = `
                    ...
                    <div class="text-right flex-shrink-0 flex gap-4">
                        <div class="text-center">
                            <p class="text-[9px] text-slate-500 uppercase font-semibold">Meta</p>
                            <p class="text-xs font-bold text-orange-400">${row.metaEst}</p>
                        </div>
                        <div class="text-center">
                ...
                `;
                listPagMeta.appendChild(itemPag);
            });
        }
